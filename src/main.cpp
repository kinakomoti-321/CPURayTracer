...
int main()
{
	...
	auto sphere2 = std::make_shared<Sphere>(Vec3(1.0,0.0,0.0),1.0f);
	auto sphere3 = std::make_shared<Sphere>(Vec3(-1.0,0.0,0.0),1.0f);
	...
	auto obj3 = std::make_shared<Object>(sphere3,mat3);
	auto obj4 = std::make_shared<Object>(sphere4,mat4);

	Scene scene(obj1,std::make_shared<Sky>(Vec3(1.0)),std::make_shared<Sphere>(Vec3(0.0),1000.0f));
	...
	const auto integrator = std::make_shared<NormalChecker>();
	auto sampler = std::make_shared<RNGrandom>();
	
	Vec3 camPos = Vec3(0.0,0.0,-3.0);
	Vec3 camLook = Vec3(0.0,0.0,1.0);

	const auto camera = std::make_shared<Pinhole>(camPos,camLook,Vec2(2.0));

	Renderer renderer(width,height,integrator,camera);
	renderer.render(scene,1,"test",sampler);
