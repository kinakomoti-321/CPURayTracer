...
int main()
{
	...
	auto sphere2 = std::make_shared<Sphere>(Vec3(0.0, 2.0, 3.0), 2.0f);
	...
	renderer.timeRender(scene, 50000, "mis_50000ms", sampler);

	renderer.reset(width, height, nee, camera);
	renderer.timeRender(scene, 50000, "nee_50000ms", sampler);

	renderer.reset(width, height, pathtracer, camera);
	renderer.timeRender(scene, 50000, "pathtrace_50000ms", sampler);
	return 0;
}